...
- **Zones freeze where they are drawn, and truncate when they die.** The three options are decided at the moment the reads disagree, so the prices are fixed at that bar and never recomputed. Each zone then truncates on the bar that **closes through** it - below a zone you would be buying into, above one you would be selling into. A wick through does not count; a close beyond the zone in the direction the trade wanted is the trade paying, not an invalidation. The episode as a whole ends when all three zones are gone, when the reads agree again, when the bias switches sides, at **`Window end (ET, HHMM)`**, or when the ET day changes (which matters when a session ends before the cutoff time - without it an episode would stretch across the overnight gap).
