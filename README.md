...
### Option zones
When the LTF CBC flips **against** the higher-timeframe bias, the method gives you three choices rather than one. This draws all three at price so you can see where each one lives. **Ships off.**

The trigger is the flip, not the standing disagreement. "CBC flips into the EMAs" names an event on one bar, so one flip opens one episode. Opening on the condition instead made every bar where the reads happened to disagree a candidate, and a failed cloud test re-armed the next one, so price wobbling across the cloud edge spawned a fresh episode on each re-entry.
...
- **Zone width** is ATR-driven: all three zones share one thickness of `Width (x ATR)` times ATR over `Width ATR period` (0.5 and 14 by default). Two of the three never had a thickness of their own, and one measured off a single prior bar collapses on a quiet bar. ATR is used rather than the distance between the EMAs because the zones only appear while the two reads *disagree*, and with the EMA cloud as the bias that disagreement clusters around the EMAs converging, so an EMA-distance width would go to zero exactly when the zones are needed. The thickness is capped so zone 1 can never spill across the flip level zone 3 marks. Widening zone 1 does not lose the precise entry edges: the BRSG zone feature already draws them on the same chart.
- **Zones freeze where they are drawn, and truncate when they die.** The three options are decided at the flip bar, so the prices are fixed there and never recomputed. Each zone then truncates on the bar that **closes through** it - below a zone you would be buying into, above one you would be selling into. A wick through does not count; a close beyond the zone in the direction the trade wanted is the trade paying, not an invalidation. The episode as a whole ends when all three zones are gone, at **`Window end (ET, HHMM)`**, or when the ET day changes (which matters when a session ends before the cutoff time - without it an episode would stretch across the overnight gap). Those are the only three, and the setup conditions are deliberately **not** among them: once the options are on the chart they stand until price invalidates them or the clock retires them. Re-testing the entry conditions every bar is what made an episode blink out and back in as price oscillated over the cloud edge.
- **Episodes may only open inside the session window**, `Window start (ET, HHMM)` to `Window end (ET, HHMM)`, 09:30 to 12:00 by default, expressed as HHMM. This is what keeps a morning worth studying on the chart: without it the afternoon's setups are always more recent than the morning's and quietly evict them from the kept examples. Preview obeys the window too, for the same reason. Set either bound to 0 to remove it.
- **Finished episodes stay on the chart as worked examples**, truncated where they died and drawn at half opacity, carrying the zone number instead of the full label. `Examples kept per side` (3 by default) holds three bullish-bias and three bearish-bias examples, so both directions are on screen to compare. Set it to 0 for the live episode only.
- The zones are **painted in `OnRender`** in the same visual language as the OrderFlowZones indicator (flat translucent fill, a lighter strip on the side price is defended from, a 1px border, an interior label), so they size in pixels and always run to the chart's right edge. They are not chart draw objects: they render beneath the indicator's own lines, which is the right layer for a translucent band, and they are not selectable or listed under Drawing Objects.
- **Preview** is a review aid, not a signal. Because an episode needs a CBC flip that runs against the bias, a closed market at the last printed bar shows nothing most of the time, which is exactly when there is time to look at them. Preview forces a bias opposite the LTF CBC so all three always draw, and skips the higher-timeframe warm-up. Only the bias is invented: the rectangles come from this chart's real prior-bar range and real 20 EMA, so size, placement and legibility are what ships. Labels are suffixed `(PREVIEW)` while it is on. Turn it off to trade.
...
- **Option zones** - Show toggle, preview toggle, bias source (HTF CBC or HTF EMA cloud), zone width (x ATR) and its ATR period, session window start/end (ET, HHMM), examples kept per side, labels toggle, fill opacity, long/short colors.
